$(document).ready(function() {
    ...
    if ($(window).width() < 993) {    // opem close dropdown nav menu on mobile devices when click (tap) on link . For destops (> 800px) used css hover property
    ...
    $("#service-page-find-master-form").submit(function() {
        $.ajax({
            type: "POST",
            url: "mail.php",
            data: $(this).serialize()
            ...
            $(this).find("input").val("");
            $("#service-page-find-master-form").trigger("reset");
            modalContentCallOrder.style.display = 'none';
            modalContentThanksMessage.style.display = 'flex';
            showModal();
        });
